...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #will change the varibale to 'i' later if required
  j=0;
  data_size = len(data);
  #print("data_size: "+str(data_size));

  while(data_index+skip_window < data_size):
    #print("data index : "+ str(data_index));
    #put data in the batch s.t. you start with context word at data_index and then, pick a word from one-index left and right
    for i in range(1,math.ceil(num_skips/2) + 1):
      #batch[j] = (data[data_index+skip_window],data[data_index+skip_window-i]);
      batch[j] = data[data_index+skip_window];
      labels[j]= data[data_index+skip_window-i];
      #print("i: " + str(i) +" context word: "+str(batch[j])+" predicted word: "+str(labels[j]));
      j +=1;
      #batch[j] = (data[data_index+skip_window], data[data_index+skip_window + i]);
      batch[j] = data[data_index + skip_window];
      labels[j] = data[data_index + skip_window + i];
      ...
      j+=1;
      if j == batch_size:
        #print('batch size attained. Will dispatch the following batch');
        #print(batch);
        #print(labels);
        data_index += 1;
        return batch,labels;
